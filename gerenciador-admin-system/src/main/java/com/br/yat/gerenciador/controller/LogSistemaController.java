...
public class LogSistemaController extends BaseController {
    ...
    public LogSistemaController(LogSistemaView view) {
        this.view = view;
        registrarAcoes();
        pesquisar(); // Carrega os logs iniciais
    }

    private void registrarAcoes() {
        view.getBtnPesquisar().addActionListener(e -> pesquisar());

        // Evento de seleção na tabela para mostrar o JSON nos detalhes
        ...
        runAsync(SwingUtilities.getWindowAncestor(view), () -> {
            ...
            for (LogSistema log : logs) {
            	view.getTableModel().addRow(new Object[]{
            		    ...
            		    log.getTipo(),
            		    log.getEntidade(), // <--- Novo campo na tabela
            		    log.getAcao(),
            		    log.isSucesso() ? "SIM" : "NÃO"
            		});
            }
            // Guarda a lista de logs na view para consulta rápida de detalhes
    ...
    private void mostrarDetalhesLog() {
        ...
        if (row >= 0) {
            ...
            view.getTxtDetalhesJson().setCaretPosition(0); // Volta pro topo do texto
        }
    }
}
