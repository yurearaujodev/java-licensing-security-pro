 ...
 * <p>
 * Fornece métodos para validar CPF, CNPJ, inscrição estadual e inscrição
 * municipal.
 * </p>
 * 
 * <p>
 * Esta classe utiliza a biblioteca <b>Stella</b>
 * ({@code br.com.caelum.stella.validation}) para validação de CPF e CNPJ,e
 * expressões regulares para inscrição estadual e municipal.
 * </p>
 * 
 * <p>
...
public final class DocumentValidator {
	...
	public static boolean isValidaCNPJ(String cnpj) {
		if (ValidationUtils.isEmpty(cnpj)) {
			return true;
		}
		try {
	 ...
	 * @return {@code true} se for um CPF ou CNPJ válido, {@code false} caso
	 *         contrário.
	 */
	public static boolean isValidaCpfCnpj(String document) {
		if (ValidationUtils.isEmpty(document)) {
			return true;
		}
		String clean = ValidationUtils.onlyNumbers(document);

		return switch (clean.length()) {
		case 11 -> isValidaCPF(clean);
		case 14 -> isValidaCNPJ(clean);
		default -> false;
		};
	 ...
	 * <p>
	 * Usa expressão regular para aceitar valores "ISENTO" ou números entre 9 e 14
	 * dígitos.
	 * </p>
	 * 
	 * @param insc inscrição estadual em formato string
	 * @return {@code true} se for válida ou estiver vazia, {@code false} caso
	 *         contrário
	 */
	public static boolean isValidInscricaoEstadual(String insc) {
		if(ValidationUtils.isEmpty(insc))return true;
		return INSCRICAO_ESTADUAL_PATTERN.matcher(insc.trim()).matches();
	}

	/**
	 * Valida incrição municipal.
	 * <p>
	 * Usa expressão regular para aceitar valores "ISENTO" ou números entre 7 e 15
	 * dígitos.
	 * </p>
	 * 
	...
	public static boolean isValidInscricaoMunicipal(String insc) {
		if(ValidationUtils.isEmpty(insc))return true;
		return INSCRICAO_MUNICIPAL_PATTERN.matcher(insc.trim()).matches();
	}
}
